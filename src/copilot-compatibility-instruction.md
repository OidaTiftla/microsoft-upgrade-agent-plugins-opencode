...
- This supersedes any preceding background `agent` or `read_agent` instructions: `agent` dispatches OpenCode `task`; `task` returns the worker result directly and synchronously. Never call or poll `read_agent`.
