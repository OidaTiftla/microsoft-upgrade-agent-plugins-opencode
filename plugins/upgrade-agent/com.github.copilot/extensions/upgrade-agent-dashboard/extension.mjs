...
async function readAssessment(activeScenario) {
  ...
  const markdown = await readAssessmentMarkdown(activeScenario);
  const detail = await readAssessmentDetailFiles(activeScenario);
  if (!file) {
    if (!markdown && detail.files.length === 0) return null;
    return {
      path: markdown?.path ?? unvalidatedString(activeScenario?.scenarioPath ?? ""),
      hasStructuredData: false,
      settings: null,
      analysisStartTime: null,
      analysisEndTime: null,
      counts: { projects: 0, issues: 0, incidents: 0, effort: 0, mandatory: 0 },
      severity: {},
      category: {},
      features: [],
      projects: [],
...
var ASSESSMENT_DETAIL_MAX_DEPTH = 8;
async function readAssessmentDetailFiles(activeScenario) {
  if (!activeScenario?.scenarioPath) return { files: [], dirs: [] };
  const root = path2.join(unvalidatedString(activeScenario.scenarioPath), "assessment");
  let rootStat;
  try {
  ...
  if (!rootStat.isDirectory()) return { files: [], dirs: [] };
  const found = [];
  const dirs = [];
  async function walk(dir, depth) {
    if (depth > ASSESSMENT_DETAIL_MAX_DEPTH) return;
    let entries;
    try {
      entries = await fs.readdir(dir, { withFileTypes: true });
    } catch {
    ...
    for (const entry of entries) {
      ...
      } else if (entry.isFile() && entry.name.toLowerCase().endsWith(".md") && entry.name.toLowerCase() !== "assessment.md") {
        found.push({ path: full, relative: path2.relative(root, full).split(path2.sep).join("/") });
      }
    }
  }
  await walk(root, 1);
  found.sort((a, b) => a.relative.localeCompare(b.relative));
  return { files: found, dirs };
}
...
function buildTokenTargets(repoRoot, scenarios, activeScenario, tasks, walkedDirs, projectFiles, scenarioChildDirs, assessmentDetailDirs) {
  ...
  if (activeScenario?.scenarioPath) {
    ...
    targets.push(path2.join(sp, "assessment"), ...assessmentDetailDirs ?? []);
  }
...
async function computeSnapshot(repoRoot, resolution, force = false) {
  ...
  const tokenTargets = buildTokenTargets(repoRoot, scenarios, activeScenario, tasks, projectsResult.walkedDirs, projectsResult.projectFiles, scenarioChildDirs, assessment?.detailDirs);
...
function snapshotNode(_kind, value) {
  return isRecord7(value) ? value : {};
}
...
function derivePhase(state) {
  const currentState = snapshotFields(state);
  const activity = unknownArray(currentState.activity).map((value) => snapshotNode("activity", value));
  const tasks = unknownArray(snapshotNode("tasks", currentState.tasks).tasks).map((value) => snapshotNode("task", value));
  const hasLiveTask = tasks.some((task) => task.state === "InProgress");
  const allTasksDone = tasks.length > 0 && tasks.every((task) => isTerminalTaskState(task.state));
  const needsAttention = tasks.some((task) => isAttentionTaskState(task.state));
  ...
  const hasAgentArtifact = activity.some(
    (entry) => entry.kind === "file" && isAgentArtifactPath(entry.filePath)
  );
...
actionHandlers.set("push_context", async (context) => {
  ...
  const payload = {
    ...
    // Counts are only meaningful when a parseable assessment.json backed them.
    // Sending zeroes for a markdown-only assessment would tell the model the
    // assessment found nothing, which is the opposite of "it was not counted".
    assessment: state.assessment ? state.assessment.hasStructuredData === false ? { path: state.assessment.path, counts: null, severity: null, structuredData: "unavailable" } : { path: state.assessment.path, counts: state.assessment.counts, severity: state.assessment.severity } : null,
