...
`com.github.copilot/extensions/upgrade-agent-dashboard/`, selected by the plugin
manifest's legacy `"extensions": "./com.github.copilot/extensions"` path. This
does not opt the plugin into the AgentPlugins specification. The generator bundles the canvas
...
Snapshot inputs remain `unknown`. `lib/snapshot-reader.ts` couples root and
declared nested field names to the producer through erased type imports, but
returns **unknown leaves**, not `DashboardSnapshot` or `Partial<DashboardSnapshot>`.
Builders narrow only the fields they consume, so a malformed artifact does not
reject the whole dashboard. Open producer dictionaries (such as scenario
properties) cannot have arbitrary key names compile-checked.

`shell-view.ts`, `activity-view.ts`, `execution-view.ts`, `diagnostics-view.ts`,
`plan.ts` and `overview.ts` own presentation projections; React owns clocks,
transport and UI state. Activity/Execution preserve already-normalized rows
instead of reformatting them. Execution alone recovers missing build verdicts
and derives repository statistics from raw activity. Overview's typed feed
retains collapse keys, repeat metadata and whole-journal duration pairing.
The browser-safety unit gate follows runtime imports, including mixed and
dynamic imports, while ignoring erased type dependencies.
